...
`Eligible_Applicants` dataset. Along the way, we create a centered form of the
`ProQOL_STS` variable that we'll use in modeling because we expect to test 
interaction terms involving that variable.  
...
Eligible_Applicants <- Applicants %>% 
  filter(Stage_Raw >= 2) %>% 
  mutate(Centered_STS = ProQOL_STS - mean(ProQOL_STS, na.rm = TRUE))
```
