...
a person completed the didactic training. The difference between the variables 
is `finisheddidac` has no missing data because applicants who were not enrolled 
are coded with 0 (did not finish). Applicants who were not enrolled have missing 
values (`NA`) on `Didactic_Status`. The crosstab output below illustrates this.

```{r}
#| label: xtab-finisheddidac-Didactic-Status

xtabs(~finisheddidac + Didactic_Status + Enrolled, addNA = TRUE, 
      data = Applicants_Raw) %>% 
  addmargins()
```

We will retain both variables because each may be useful depending on the 
summary desired.
:::
...
analyses by applying more consistent and convenient naming conventions, 
converting categorical variables to factors, and so on. 
