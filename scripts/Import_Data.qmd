...
Stage_Reached_value_labels <- c(`Eligible` = 1, `Started DT` = 2, 
...
tibble(Raw_Value = 1:7,
       Reached_Value = c(NA, 1, 1, 2, 3, 3, 4)) %>% 
...
The rationale for collapsing the `Stage_Raw` values of 2 and 3 together is that 
the first stopping rate of interest is the proportion of eligible applicants 
who drop out before starting the didactic training. As a practical matter from a 
program management perspective, the PIs decided that distinguishing between
being eligible (`Stage_Raw` = 2) and enrolled (`Stage_Raw` = 2) was not very
useful because both values still ultimately mean the person did not start the 
training. Only one participant started the clinical skills training without
...
Below we also need to construct a scale measuring the extent to which personal
and professional demands on participants' time are a barrier to participation in
the training program (`Barrier_TD`). We considered using the following three 
items from the training application form:
...
construct the sum score for the remaining two items, but the descriptive 
analyses script will assess its reliability. 
...
Applicants <- Applicants_Raw %>% 
  ...
  rowwise() %>% 
  mutate(Barrier_TD = sum(c(Train_Goal_Fac_Bar_8, Train_Goal_Fac_Bar_10), 
                          na.rm = FALSE)) %>% 
  ungroup() %>% 
  ...
  set_variable_labels(ID = "Applicant ID (may be blank if application was incomplete)",
                      ...
                      Barrier_TD = "Barrier Time Demands Scale Score (3-item sum, possible range [2, 10])") %>% 
...
We need to subset the data to create an `Eligible_Applicants` dataset so we can 
focus on the intended population to which we want to generalize. 
...
However, we also need to assess whether these eligible applicants all have 
complete data on the variables to be used in our models. 
...
#| label: tbl-Eligible-Applicants-cvv-missingness
 ...
 Eligible_Applicants %>% 
  select(ID, Stage_Reached, Barrier_TD, ProQOL_BO, ProQOL_CS, ProQOL_STS,
         Motivation_NeedSANE, Motivation_PersonalConn, Setting) %>% 
  cvv_missingness() %>% 
  kable(format = "latex", booktabs = TRUE, digits = 1) %>% 
  kable_styling() 
```

We can see that a few eligible applicants have any missing data on the variables 
we want to use in the models. @tbl-Eligible-Applicants-var-missingness shows 
variable-specific rates of missingness. 
...
mean-centered versions of the continuous predictors that can be idenified by 
a `C` prefix on the variable name`. 

```{r}
#| label: create-Eligible-Applicants-CD

Eligible_Applicants_CD <- Eligible_Applicants %>% 
  # Retain cases with no missing data on variables to be used in models. 
  filter(if_all(c(ID, Stage_Reached, Barrier_TD, ProQOL_BO, ProQOL_CS, 
                  ProQOL_STS, Motivation_NeedSANE, Motivation_PersonalConn, 
                  Setting), ~ !is.na(.x))) %>% 
  # Create mean-centered versions of continuous predictors. 
  mutate(CProQOL_BO = ProQOL_BO - mean(ProQOL_BO),
         CProQOL_CS = ProQOL_CS - mean(ProQOL_CS),
         CProQOL_STS = ProQOL_STS - mean(ProQOL_STS),
         CBarrier_TD = Barrier_TD - mean(Barrier_TD))
```
...
vertically stack the threshold-specific datasets (`T1` to `T4`). Along the way, 
we create factor versions of the categorical variables that can be identifed by 
the `.` suffix on the variable name. 

```{r}
#| label: create-T1

T1 <- Eligible_Applicants_CD %>% 
  select(ID, Stage_Reached, Barrier_TD, ProQOL_BO, ProQOL_CS, ProQOL_STS, 
         CBarrier_TD, CProQOL_BO, CProQOL_CS, CProQOL_STS, 
         Motivation_NeedSANE, Motivation_PersonalConn, Setting) %>% 
...
# Create vectors of value labels for reverse-coded NPQ items.
Threshold_Labels <- c("T1 (Eligible/Enrolled)" = 1,
                      "T2 (Enrolled/Started DT)" = 2,
                      "T3 (Started DT/Finished DT)" = 3,
                      "T4 (Finished DT/Finished CSW)" = 4)
```
...
`Eligible_Applicants` the stopped cases are those that enrolled but lacked 
complete data on variables required for modeling. For `Eligible_Applicants_CD`, 
stopped cases were those who did not enroll. `T1` to `T4` each have one row per
person, with the stopped cases being those that attrited at the specified
threshold. The `Thresholds` dataset contains one row per person per threshold
encountered because it combined the rows of `T1` to `T4`, with the stopped cases
being all the rows that indicated attrition occurred.
...
tibble(DataFrame = c("Applicants", "Eligible_Applicants", 
                     "Eligible_Applicants_CD", "T1", "T2", "T3", "T4", 
                     "Thresholds"),
       N_Rows = c(nrow(Applicants), nrow(Eligible_Applicants), 
                  nrow(Eligible_Applicants_CD), nrow(T1), nrow(T2), nrow(T3), 
                  nrow(T4), nrow(Thresholds)),
       N_Cols = c(ncol(Applicants), ncol(Eligible_Applicants), 
                  ncol(Eligible_Applicants_CD), ncol(T1), ncol(T2), ncol(T3), 
                  ncol(T4), ncol(Thresholds)),
       Stopped = c(sum(is.na(Applicants$Stage_Reached)),
                   sum(is.na(Eligible_Applicants$ProQOL_BO)),
                   sum(Eligible_Applicants_CD$Stage_Reached < 2),
...
save_what <- c("Applicants", "Eligible_Applicants", "Eligible_Applicants_CD", 
               "Thresholds")
```
