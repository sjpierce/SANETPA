...
the training program (`Barrier_TD`) using the following three items from the
...
* `train_goal_fac_bar_5_numeric`: I have a lot of family obligations right now. 
...
agree). We want to create an unweighted sum of the items such that large values 
indicate high external demands on the participant's time, so we must reverse 
...
disagree) for that to make sense. This script will construct the sum score, but 
the descriptive analyses script will assess its reliability. 
...
Applicants <- Applicants_Raw %>% 
  ...
  rowwise() %>% 
  mutate(Barrier_TD = sum(c(Train_Goal_Fac_Bar_5R, Train_Goal_Fac_Bar_8,
                            Train_Goal_Fac_Bar_10), na.rm = FALSE)) %>% 
  ungroup() %>% 
  ...
  set_variable_labels(ID = "Applicant ID (may be blank if application was incomplete)",
                      ...
                      Barrier_TD = "Demands on Time Scale Score (3-item sum, possible range [3, 15])") %>% 
...
## Create Eligible Applicants Dataset
...
`Eligible_Applicants` dataset. Along the way, we create centered forms of the
continuous predictors that we'll use in modeling because we expect to test 
interaction terms involving them.  
...
Eligible_Applicants <- Applicants %>% 
  ...
  mutate(CProQOL_BO = ProQOL_BO - mean(ProQOL_BO, na.rm = TRUE),
         CProQOL_CS = ProQOL_CS - mean(ProQOL_CS, na.rm = TRUE),
         CProQOL_STS = ProQOL_STS - mean(ProQOL_STS, na.rm = TRUE),
         CBarrier_TD = Barrier_TD - mean(Barrier_TD, na.rm = TRUE))
```
