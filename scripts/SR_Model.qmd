...
library(piercer)        # for brier(), file_details(), git_report(), R2Dev()
...
parallel effect model is nested within the non-parallel effect mode, we can 
...
in second language learning [@Winke-RN3795], and predictors of how far sexual 
assault kits progressed through several stages of forensic testing 
...
data on all the measures listed in the next section. 

## Measures
The measures mentioned below include an outcome variable, a structural predictor
required for SR modeling, and seven focal predictors.
...
### Barrier Time Demands (Focal Predictor)
This is a mean-centered, continuous variable (`Barrier_TD`) created for this 
study. It is based on the sum of two Likert-response ordinal items that were 
scored on a 5-point scale. See the descriptive analyses output file for this 
...
Our analyses showed no evidence that the size remaining focal predictors listed
