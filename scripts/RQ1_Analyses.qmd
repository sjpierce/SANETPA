...
Eligible_Applicants <- Eligible_Applicants %>% 
  mutate(Education = as_factor(Education),
         ...
         Enrolled = as_factor(Enrolled))
```

\FloatBarrier

# Methods
Estimating the eligibility and enrollment rates is simple: we simply take the 
means of relevant binary variables representing which applicants were eligible 
and which ones enrolled. Starting from a dataset with the correct set of 
applicants ensures that we use the correct denominator for each rate (remember 
that the mean of a binary variable is the proportion of observations that have a 
value of 1). The eligibility rate must use data about all applicants, whereas 
the enrollment rate must use only data from eligible applicants. We used Wilson 
score confidence intervals to quantify the uncertainty in both rates 
