...
`Eligible_Applicants` is the dataset before listwise deletion of applicants with 
missing data on any variable to be used in the models. It has 
`r nrow(Eligible_Applicants)` rows. `Eligible_Applicants_CD` is the dataset of 
eligible applicants with complete data (remaining after listwise deletion). It 
has `r nrow(Eligible_Applicants_CD)` rows.
