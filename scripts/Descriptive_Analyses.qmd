...
Below we esimate the reliability of a scale measuring the extent to which 
personal and professional demands on participants' time are a barrier to 
participation in the training program (`Barrier_TD`) using the following three 
items from the training application form:

* `Train_Goal_Fac_Bar_5R`: I have time in my schedule to focus on this 
  course.  
...
Given the oor to questionable reliability demonstrated by the results above, I
have asked the investigators whether they still want me to use `Barrier_TD` as a
...
@tbl-continuous-vars shows descriptive statistics for each of these 
continuous predictors, along with corresponding mean-centered versions of each 
one. Histograms of the undecentered versions of the variables are shown in 
@fig-ProQOL-BO, @fig-ProQOL-CS, @fig-ProQOL-STS, and @fig-Barrier-TD.

```{r}
#| label: tbl-continuous-vars
...
```{r}
#| label: fig-ProQOL-BO
#| fig-cap: Histogram of ProQOL Burnout Among Eligible Applicants
#| fig-width: 5
#| fig-height: 2.5
#| warning: false

ggplot(data = Eligible_Applicants, aes(x = ProQOL_BO)) +
...
```{r}
#| label: fig-ProQOL-CS
#| fig-cap: Histogram of ProQOL Compassion Satisfaction Among Eligible Applicants
#| fig-width: 5
#| fig-height: 2.5
#| warning: false

ggplot(data = Eligible_Applicants, aes(x = ProQOL_CS)) +
  geom_histogram(aes(y = after_stat(density)), binwidth = 1, color = "black", 
                 fill = "grey") +
  geom_density(alpha = .6, fill = "#FF6666") +
...
#| fig-cap: Histogram of ProQOL Secondary Traumatic Stress Among Eligible Applicants
#| fig-width: 5
#| fig-height: 2.5
#| warning: false

ggplot(data = Eligible_Applicants, aes(x = ProQOL_STS)) +
  geom_histogram(aes(y = after_stat(density)), binwidth = 1, color = "black", 
                 fill = "grey") +
  geom_density(alpha = .6, fill = "#FF6666") +
...
#| fig-cap: Histogram of Barrier Time Demands Among Eligible Applicants
#| fig-width: 5
#| fig-height: 2.5

ggplot(data = Eligible_Applicants, aes(x = Barrier_TD)) +
  geom_histogram(aes(y = after_stat(density)), binwidth = 1, color = "black", 
                 fill = "grey") +
  geom_density(alpha = .6, fill = "#FF6666") +
  geom_vline(aes(xintercept = mean(Barrier_TD, na.rm = TRUE)), color = "blue", 
             linetype = "solid", linewidth = 1) +
  coord_cartesian(xlim = c(3, 15), ylim = c(0, 0.30)) + 
  scale_x_continuous(breaks = seq(from = 3, to = 15, by = 1)) +
...
#| tbl-cap: Pearson Correlation Matrix for Continuous Predictor Variables 

N_used <- Eligible_Applicants %>% 
