...
@tbl-Setting shows the frequency distribution for the setting stage where 
eligible applicants practice. 

```{r}
#| label: tbl-Setting
#| tbl-cap: Practice Setting Frequency Distribution Among Eligible Applicants

# Footnote text. 
FN <- paste("Denominator for percents is total number of eligible applicants.")

xtabs(~as_factor(Setting), addNA = TRUE, 
      data = Eligible_Applicants) %>% 
  addmargins() %>% 
  as.data.frame() %>% 
  ...
  kable(format = "html", booktabs = TRUE, digits = 1,
        col.names = c("Setting", "Freq", "%", "Cumulative %")) %>% 
  kable_styling() %>% 
