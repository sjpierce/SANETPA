...
avg_predictions(m2, variables = c("Threshold.")) %>% 
  as_tibble() %>% 
  select(Threshold., estimate, conf.low, conf.high) %>% 
  mutate(Threshold. = factor(Threshold., labels = TLabels2)) %>% 
  ggplot(data = ., mapping = aes(x = Threshold., y = estimate)) + 
...
but Model 2 provides a better answer. Model 2 output (@tbl-emmeans-m2-Both and @fig-emmeans-m2-Both) controls for all focal predictors regardless of whether 
their effects were significant. *In that context, we still see a much higher *
*attrition rate during didactic training than at other thresholds*. 
