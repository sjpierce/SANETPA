...
[@Dontje-RN8757]. The training program had two major components: didactic 
training (DT) followed by a clinical skills workshop (CSW). 
    ...
    * `scripts/` The folder is configured as a Quarto project. It holds 
      meta-data files, `.qmd` scripts, and files used by the scripts. 
        ...
        * `output/`: This subfolder holds rendered output files. 
            * `CR_Model_Draft.pdf`: This is temporary draft output obtained 
        ...
        * `CR_Model.qmd`: This script runs the continuation-ratio model(s) that 
          ...
          `output/CR_Model_Draft.pdf`; you can get date-stamped production 
...
If you want to get a date-stamped production run output, you can use something 
like the following command in the Terminal to change into the `scripts/` 
folder and render that output. Just update the date in the output filename to 
match the current date (a new output file will overwrite an existing output file 
